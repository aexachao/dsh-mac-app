...
- **Logging**: stdout and stderr share one pipe, drained on the `dsh-web.log` background queue, split on newlines, hopped to the main actor via `ingest`, timestamped, capped at 5000 lines. Entering `.failed` auto-expands the log sidebar (`ContentView.onChange(of: server.state)`). Log lines are not yet masked or persisted.
